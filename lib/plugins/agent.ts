...
export class AgentPlugin implements Plugin {
  ...
  install(bus: EventBus): void {
    ...
    switch (mode) {
      case "ralph":
        ...
        debug("Agent plugin: Ralph session strategy (fresh per trigger)");
        break;
      case "long-lived":
      default:
  ...
  private async handleInbound(bus: EventBus, msg: BusMessage): Promise<void> {
    ...
    const response = await this.strategy.run(`signal:${sender}`, content);
  ...
  private async handleCron(bus: EventBus, msg: BusMessage): Promise<void> {
    ...
    const response = await this.strategy.run(channelId, content);
    if (!response) return;
